...
class Detector:
    def __init__(self, conf_thres: float = 0.25, iou_thresh: float = 0.45, agnostic_nms: bool = False,
                 save_conf: bool = False, classes: list = None):
        """
        args:
        conf_thres: Thresholf for Classification
        iou_thres: Threshold for IOU box to consider
    ...
    def detect(self, source, plot_bb: bool = True):
        ...
        # Apply Classifier
        if self.classify:
            pred = apply_classifier(pred, self.modelc, img, im0)  # I think we need to add a new axis to im0
        ...
        return im0 if plot_bb else None  # just in case there's no detection, return the original image. For tracking
        # purpose plot_bb has to be False always
